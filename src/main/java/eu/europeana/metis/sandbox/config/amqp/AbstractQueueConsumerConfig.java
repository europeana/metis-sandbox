...
/**
 * The type Queue consumer configuration for workflow listeners. Every listener has a
 * {@link SimpleRabbitListenerContainerFactory}. <br /> If changes like increasing concurrency for a listener are needed, every
 * subclass can be customized by using the SimpleRabbitListenerContainerFactory.
 */
@SuppressWarnings("java:S1694") // Intentionally abstract to prevent instantiation and promote extension only.
public abstract class AbstractQueueConsumerConfig {
   ...
   */
  protected AbstractQueueConsumerConfig(MessageConverter messageConverter) {

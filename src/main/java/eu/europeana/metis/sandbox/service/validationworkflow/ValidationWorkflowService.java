...
public class ValidationWorkflowService {
  ...
  public ValidationWorkflowService(
      @Qualifier("harvestValidationStep") ValidationStep harvestValidationStep,
      @Qualifier("externalValidationStep") ValidationStep externalValidationStep,
      @Qualifier("transformationValidationStep") ValidationStep transformationValidationStep,
   ...
   */
  public ValidationWorkflowReport validate(MultipartFile recordToValidate, Country country, Language language)
  ...
  private Record createDatasetAndGetHarvestedRecord(MultipartFile recordToValidate, Country country, Language language)
      throws IOException {
  ...
  private void finalizeValidationPatternAnalysis(String datasetId, ExecutionPoint datasetExecutionPoint) {
    final Lock lock = datasetIdLocksMap.computeIfAbsent(datasetId,
        s -> lockRegistry.obtain("finalizeValidationPatternAnalysis_" + datasetId));
    try {
      lock.lock();
  ...
  private List<ValidationResult> performSteps(Record recordToProcess) {
    ...
    ValidationStepContent externalValidationStepContent = externalValidationStep.performStep(
        harvestStepContent.getRecordStepResult());
    ...
    ValidationStepContent transformationStepContent = transformationValidationStep.performStep(
        externalValidationStepContent.getRecordStepResult());
    ...
    ValidationStepContent internalValidationStepContent = internalValidationValidationStep.performStep(
        transformationStepContent.getRecordStepResult());

...
public interface AsyncDatasetPublishService {
  ...
  /**
   * Async publish to message broker for further processing. This will send messages to 'created` queue
   * to skip the transformation to edm external step
   *
  ...
  /**
   * Async publish to message broker for further processing. This will send messages to 'transformationToEdmExternal' queue
   * to go through the transformation to edm external step
   *
   * @param dataset to publish. Must not be null
   * @return {@link CompletableFuture} of the process
   * @throws NullPointerException     if dataset object is null
